[![Docker Repository on Quay](https://quay.io/repository/prometheuscommunity/oidc-prom-label-proxy/status "Docker Repository on Quay")](https://quay.io/repository/prometheuscommunity/oidc-prom-label-proxy)

# oidc-prom-label-proxy

The oidc-prom-label-proxy can enforce a given label in a given PromQL query, in Prometheus API responses or in Alertmanager API requests. As an example (but not only),
this allows read multi-tenancy for projects like Prometheus, Alertmanager or Thanos.

This proxy does not perform authentication or authorization, this has to happen before the request reaches this proxy, allowing you to use any authN/authZ system you want. The [kube-rbac-proxy](https://github.com/brancz/kube-rbac-proxy) is an example for such an additional building block. Additionally, you can use oidc-prom-label-proxy as a library in your own proxy, like what is done in [prom-authzed-proxy](https://github.com/authzed/prom-authzed-proxy).
...
## Installing `oidc-prom-label-proxy`
...
If you want to build `oidc-prom-label-proxy` from source you would need a working installation of
...
You can run `oidc-prom-label-proxy` to enforce the value of the `tenant` label
...
{"error":"The \"tenant\" query parameter must be provided.","errorType":"oidc-prom-label-proxy","status":"error"}
...
Alternatively, `oidc-prom-label-proxy` can use a custom HTTP header instead HTTP parameters:
...
Now oidc-prom-label-proxy enforces the `tenant="prometheus"` label in all requests.
...
`oidc-prom-label-proxy` will enforce the `tenant=~"prometheus|alertmanager"` label selector in all requests.
...
NOTE: When the `/api/v1/labels` and `/api/v1/label/<name>/values` endpoints were added to `oidc-prom-label-proxy`, the Prometheus and Thanos endpoints didn't support the `match[]` parameter hence the `oidc-prom-label-proxy` labels endpoints are disabled by default. Use the `-enable-label-apis` flag to enable with care. Ensure that the upstream endpoints support label selectors:
...
:rotating_light: `oidc-prom-label-proxy` doesn't support multiple label values for the Silences endpoints :rotating_light:
